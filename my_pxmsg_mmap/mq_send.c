...
mymq_send(mymqd_t mqd, const char *ptr, size_t len, unsigned int prio)
{
	...
	else if (attr->mq_curmsgs >= attr->mq_maxmsg) // 检查队列是否填满
	{
		/* queue is full */
...
/* end mq_send1 */
/* include mq_send2 */
	/* nmsghdr will point to new message. 获取待用空闲块的索引 */
	...
	/* wake up anyone blocked in mq_receive waiting for a message. 唤醒阻塞在mymq_receive中的任何线程 */
	if (attr->mq_curmsgs == 0)
		pthread_cond_signal(&mqhdr->mqh_wait);
	attr->mq_curmsgs++;// 给当前在该队列中的消息数加 1 .
